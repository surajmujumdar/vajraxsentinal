...
export function ThreatIntelView() {
  const [findings, setFindings] = useState(() => filterModuleFindings('threat_intel', dashboardService.getInitialFindings({ module: 'threat_intel' })));
  const [selectedRating, setSelectedRating] = useState('ALL');
  const [searchQuery, setSearchQuery] = useState('');
  const [selectedFinding, setSelectedFinding] = useState(() => {
    const init = filterModuleFindings('threat_intel', dashboardService.getInitialFindings({ module: 'threat_intel' }));
  ...
  useEffect(() => {
    async function loadIntel() {
      ...
      if (intelFindings.length > 0) {
        setSelectedFinding(prev => {
          if (prev && intelFindings.some(f => String(f.id) === String(prev.id))) {
            return intelFindings.find(f => String(f.id) === String(prev.id)) || prev;
          }
          return intelFindings[0];
        });
      }
    }
    loadIntel();
          ...
          <div className="cyber-card" style={{ padding: '14px', background: '#060108', border: '2.5px solid #360a25', maxHeight: '720px', overflowY: 'auto' }}>
            ...
            <div style={{ marginBottom: '10px' }}>
              ...
              >
                ...
                <input
                  type="text"
                  placeholder="Filter threat alerts..."
                  value={searchQuery}
                  ...
                  return (
                    <div
                      ...
                      style={{
                        padding: '10px 12px',
                        borderRadius: '6px',
                        background: isSelected ? 'rgba(251, 191, 36, 0.18)' : '#040005',
                        border: isSelected ? '1.5px solid #fbbf24' : '1.5px solid #28081c',
                        boxShadow: isSelected ? '0 0 12px rgba(251, 191, 36, 0.25)' : 'none',
                        cursor: 'pointer',
          ...
          {activeSelected && (
            <div className="cyber-card" style={{ padding: '20px', background: '#060108', border: '3px solid #360a25', display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px', flexWrap: 'wrap' }}>
                    ...
                    <span style={{ fontSize: '11px', color: '#fbbf24', fontFamily: 'var(--font-mono)', padding: '1px 6px', background: 'rgba(251, 191, 36, 0.12)', borderRadius: '3px', border: '1px solid #fbbf24' }}>
                      {activeSelected.category || 'THREAT INTEL'}
                    </span>
                    <span style={{ fontSize: '10.5px', color: '#fbbf24', fontFamily: 'var(--font-mono)' }}>
                      THREAT FEED MATCH
                    </span>
                  </div>
                  ...
                  <div style={{ fontSize: '11.5px', color: '#fbbf24', fontFamily: 'var(--font-mono)' }}>
                    Target Indicator: {activeSelected.affectedComponent}
                  </div>
                </div>
              ...
              <div>
                ...
                >
                  {activeSelected.rawEvidenceSnippet || activeSelected.evidence || activeSelected.code_snippet || getFindingCodeSnippet(activeSelected)}
                </div>
              </div>
              ...
              <div style={{ padding: '10px 14px', borderRadius: '6px', background: 'rgba(0, 255, 136, 0.08)', border: '1.5px solid #00ff88', fontSize: '11.5px', color: '#f8fafc' }}>
                ...
                {activeSelected.aiAnalysis?.recommendation || activeSelected.remediation || getFindingRemediation(activeSelected)}
              </div>
            </div>
          )}
        </div>
