...
export const Sidebar = ({ currentTab, onTabChange }) => {
  ...
  const renderItem = (item, isEngine = false) => {
    ...
    const containerClasses = isActive
      ? `group flex items-center justify-between px-3.5 py-2.5 rounded-xl ${activeBg} border-l-4 border-y border-r ${activeBorderColor} text-white shadow-[inset_0_0_18px_rgba(255,23,68,0.2)] transition-all cursor-pointer`
      : "group flex items-center justify-between px-3.5 py-2.5 rounded-xl text-slate-300 hover:text-rose-200 hover:bg-rose-950/40 border border-transparent hover:border-rose-500/25 transition-all cursor-pointer";
      ...
      <a key={item.id + (isEngine ? '-eng' : '')} onClick={() => onTabChange(item.id)} className={containerClasses}>
        ...
        {item.statusLabel && (
          <span className="flex items-center space-x-1.5 flex-shrink-0">
            <span className={`w-2 h-2 rounded-full bg-rose-500 ${item.isPulse ? 'animate-pulse' : ''} shadow-[0_0_8px_#ff1744]`}></span>
            <span className={`text-[10px] text-rose-300 font-bold uppercase tracking-wider`}>{item.statusLabel}</span>
          </span>
        )}
  ...
  return (
    <aside className="w-full lg:w-80 xl:w-[340px] flex-shrink-0 tech-border-card rounded-2xl border border-rose-500/30 bg-command-900/95 backdrop-blur-md shadow-[0_0_45px_rgba(7,1,4,0.95)] sticky top-20 z-30 p-5" data-purpose="platform-modules-sidebar">
      {/* Sidebar Header / Module Crest */}
      <div className="flex items-center justify-between pb-3.5 border-b border-rose-900/50 mb-4">
        <div className="flex items-center space-x-3">
          <div className="w-3 h-3 bg-rose-500 rounded-sm shadow-[0_0_10px_#ff1744]"></div>
          <div>
            <span className="font-hud font-bold tracking-widest text-sm uppercase text-white drop-shadow-[0_0_8px_rgba(255,23,68,0.5)]">
              PLATFORM MODULES
            </span>
      ...
      <nav className="space-y-1.5 font-mono text-xs max-h-[calc(100vh-210px)] overflow-y-auto hud-scrollbar pr-1">
        ...
        <div className="pt-3 pb-1.5 px-3">
          <div className="flex items-center justify-between text-[10px] text-rose-400/70 uppercase tracking-widest border-t border-rose-900/50 pt-3">
      ...
      <div className="mt-5 pt-3.5 border-t border-rose-900/50 bg-command-950/70 rounded-xl p-3 border border-rose-900/40">
